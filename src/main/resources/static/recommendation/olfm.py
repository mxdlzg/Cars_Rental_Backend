...
def getUserNegativeItem(frame, userID):
    ...
    :return: 负反馈物品
    '''
    userItemlist = list(set(frame[frame['user_id'] == userID]['car_id']))                       #用户评分过的物品
    otherItemList = [item for item in set(frame['car_id'].values) if item not in userItemlist] #用户没有评分的物品
    itemCount = [len(frame[frame['car_id'] == item]['user_id']) for item in otherItemList]      #物品热门程度
...
def getUserPositiveItem(frame, userID):
    ...
    :return: 正反馈物品
    '''
    series = frame[frame['user_id'] == userID]['car_id']
...
def initModel(frame, classCount):
    ...
    itemID = list(set(frame['car_id'].values))
...
def recommend(frame, userID, p, q, TopN=10):
    ...
    :return: 推荐物品
    '''
    userItemlist = list(set(frame[frame['user_id'] == userID]['car_id']))
    otherItemList = [item for item in set(frame['car_id'].values) if item not in userItemlist]
...
if __name__ == '__main__':
    frame = getResource('data/ratings.csv')
    p, q = latenFactorModel(frame, 5, 10, 0.02, 0.01)
    l = recommend(frame, 8, p, q)
    print(l)